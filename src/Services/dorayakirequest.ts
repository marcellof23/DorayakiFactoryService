...
import { DorayakiRequest as DorayakiRequestModel, Recipe as RecipeModel, RecipeIngredient, Ingredient } from "Models/";
...
export const update_request = async (req: Request, res: Response) => {
	try {
		...
		const dr: DorayakiRequestModel = await DorayakiRequest.findByPk(dorayakirequest_id);
		...
		if (error) return sendRes(res, 400, DORAYAKIREQUEST_LOG.PUT[400][0], error.details[0].message);
		...
		if (status === DorayakiRequestStatus.accepted) {
			...
			for (const ingredient of ingredients) {
				await ingredient.update({
					stock: ingredient.stock - ingredient.RecipeIngredient.qty_required * dr.qty
				})
			}
		}
